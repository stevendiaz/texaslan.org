...
class PledgeOrActiveRequiredMixin(UserPassesTestMixin):
    def test_func(self):
        return not self.request.user.is_anonymous and (self.request.user.is_pledge() or self.request.user.is_active_user())
